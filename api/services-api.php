        ...
        switch ($action) {
                ...
                try {
                    ...
                    if ($tablesExist['services']) {
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            $nhis_tariff = isset($row['nhis_tariff']) ? (float)$row['nhis_tariff'] : 0;
                            $private_price = isset($row['private_price']) ? (float)$row['private_price'] : 0;
                            $nhis_covered = isset($row['nhis_covered']) ? (bool)$row['nhis_covered'] : true;
                            
                            // Determine the tariff to use - prefer the higher price if both exist
                            $tariff = 0;
                            if ($nhis_covered && $nhis_tariff > 0) {
                                $tariff = $nhis_tariff;
                            } elseif ($private_price > 0) {
                                $tariff = $private_price;
                            } elseif ($nhis_tariff > 0) {
                                $tariff = $nhis_tariff;
                            }
                            
                            // If tariff is still 0, set a default price based on service category
                            if ($tariff <= 0) {
                                $category = strtolower($row['category'] ?? 'general');
                                if (in_array($category, ['consultation', 'procedure'])) {
                                    $tariff = 25.00; // Default consultation price
                                } elseif (in_array($category, ['laboratory', 'radiology'])) {
                                    $tariff = 15.00; // Default lab price
                                } else {
                                    $tariff = 20.00; // Default general service price
                                }
                            }
                    ...
                    if ($tablesExist['medications']) {
                        try {
                            ...
                            while ($row = $medStmt->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                // Get the unit price, ensuring it's not 0
                                $unitPrice = isset($row['unit_price']) ? (float)$row['unit_price'] : 0;
                                if ($unitPrice <= 0) {
                                    $unitPrice = 10.00; // Default price for medications
                                }
                    ...
                    if ($tablesExist['lab_tests']) {
                        try {
                            ...
                            while ($row = $labStmt->fetch(PDO::FETCH_ASSOC)) {
                                // Get the lab test price, ensuring it's not 0
                                $labPrice = isset($row['price']) ? (float)$row['price'] : 0;
                                if ($labPrice <= 0) {
                                    $labPrice = 15.00; // Default price for lab tests
                                }
                    ...
                    // Add debug info if debug parameter is present
                    if (isset($_GET['debug']) && $_GET['debug'] === '1') {
                        $response['debug'] = [
                            'tables_exist' => $tablesExist,
                            'service_count' => $serviceCount ?? 0,
                            'medication_count' => $medicationCount ?? 0,
                            'lab_test_count' => $labTestCount ?? 0,
                            'total_services' => [
                                'opd' => count($groupedServices['opd']),
                                'lab' => count($groupedServices['lab']),
                                'pharmacy' => count($groupedServices['pharmacy'])
                            ]
                        ];
                    }
            ...
            case 'debug':
                try {
                    ...
                    $checkStmt = $conn->prepare($checkQuery);
                    $checkStmt->execute();
                    $debug_info['table_exists'] = $checkStmt->rowCount() > 0;
                    ...
                    if ($debug_info['table_exists']) {
                        ...
                        $columnsStmt = $conn->prepare($columnsQuery);
                        $columnsStmt->execute();
                        $debug_info['columns'] = $columnsStmt->fetchAll(PDO::FETCH_ASSOC);
                        
                        // Get row count
                        $countQuery = "SELECT COUNT(*) FROM services";
                        $countStmt = $conn->prepare($countQuery);
                        $countStmt->execute();
                        $debug_info['row_count'] = $countStmt->fetchColumn();
                        
                        // Get sample data
                        $sampleQuery = "SELECT * FROM services LIMIT 5";
                        $sampleStmt = $conn->prepare($sampleQuery);
                        $sampleStmt->execute();
                        $debug_info['sample_data'] = $sampleStmt->fetchAll(PDO::FETCH_ASSOC);
                    }
