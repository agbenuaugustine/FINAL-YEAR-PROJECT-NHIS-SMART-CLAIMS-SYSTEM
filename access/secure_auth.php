...
if (time() - $_SESSION['last_activity'] > $timeout) {
    ...
    session_unset();
    session_destroy();
    header('Location: /smartclaimsCL/index.php?error=session_expired');
    exit();
}
...
$allowed_roles = [
    ...
    'patient-registration.php' => ['superadmin', 'hospital_admin', 'admin', 'doctor', 'nurse', 'receptionist', 'records_officer'],
    'visits.php' => ['superadmin', 'hospital_admin', 'admin', 'doctor', 'nurse', 'receptionist'],
    'vital-signs.php' => ['superadmin', 'hospital_admin', 'admin', 'doctor', 'nurse'],
    'diagnosis.php' => ['superadmin', 'hospital_admin', 'admin', 'doctor'],
    'claims.php' => ['superadmin', 'hospital_admin', 'admin', 'claims_officer', 'doctor'],
...
if (!isset($_SESSION['user']['is_active'])) {
    ...
    session_unset();
    session_destroy();
    header('Location: /smartclaimsCL/index.php?error=account_deactivated');
    exit();
...
$current_page = basename($_SERVER['PHP_SELF']);
$skip_csrf_pages = ['user-management.php']; // Pages that handle their own CSRF

if ($_SERVER['REQUEST_METHOD'] === 'POST' && !in_array($current_page, $skip_csrf_pages)) {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        // CSRF token mismatch
        header('Location: /smartclaimsCL/index.php?error=csrf_error');
        exit();
    }
}
